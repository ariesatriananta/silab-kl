...
export function ToolsPageClient({ data }: { data: ToolRow[] }) {
  ...
  const labs = Array.from(new Set(data.map((t) => t.lab)))
                ...
                {filtered.map((tool) => {
                  ...
                  return (
                    <TableRow key={tool.id} className="hover:bg-muted/30">
                      ...
                      <TableCell className="text-muted-foreground capitalize">{tool.condition}</TableCell>
                      <TableCell>
      ...
      <Dialog open={!!showQrDialog} onOpenChange={() => setShowQrDialog(null)}>
        <DialogContent className="max-w-xs">
          <DialogHeader>
            ...
            <DialogDescription>Cetak QR code ini untuk identifikasi alat.</DialogDescription>
          </DialogHeader>
